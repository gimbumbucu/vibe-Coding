...
def get_payment_gateway_service() -> IPaymentGatewayService:
    ...
    if not TOSS_SECRET_KEY or "your-toss" in TOSS_SECRET_KEY:
        raise RuntimeError("TOSS_SECRET_KEY must be configured in .env for payments")
    _payment_gateway_service = TossPaymentGatewayService(TOSS_SECRET_KEY)
    return _payment_gateway_service
